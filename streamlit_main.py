...
def load1():
    ...
    data_1["MINIMUM_PAYMENTS"].fillna(data_1["MINIMUM_PAYMENTS"].median(),
                                      inplace=True)
    return data_1
...
def load2():
    ...
    DA['MINIMUM_PAYMENTS'].fillna(DA['MINIMUM_PAYMENTS'].median(),
                                  inplace=True)

    dat = DA.values[3000:4000]
    return dat
...
def show2():
    ...
    dat = PCA(n_components=7).fit_transform(dat)
    dat = (dat - min(dat, axis=0)) / (max(dat, axis=0) - min(dat, axis=0))

    percent = 0.01
    dis = get_distance(dat)
    dc = get_dc(dis, percent)
    local_density = get_local_density(dis, dc)
    deltas, nearest_neighbor = get_deltas(dis, local_density)
    centers = find_k_centers(local_density, deltas, 3)
    labels = density_peal_cluster(local_density, centers, nearest_neighbor)

    df = pd.DataFrame(dat, columns=["x", "y", "z", "a", 'b', 'c','e'])
    ...
    chart = altair.Chart(df).mark_point(shape='diamond').encode(
        x='a', y='b', color='label').interactive()

    center_chart = altair.Chart(center_df).mark_point().encode(
        x='a', y='b', color="label", size='font').interactive()
    ...
    st.subheader("")
    st.subheader("用户聚类")
    st.altair_chart(chart + center_chart + sig_chart,
                    theme="streamlit",
...
def M():
    ...
    data["MINIMUM_PAYMENTS"].fillna(data["MINIMUM_PAYMENTS"].median(),
                                    inplace=True)
    ...
    with st.sidebar:
        ...
        if uploaded_file is not None:
            ...
            col1.metric(
                "余额", csv_data.iloc[0, 1], "-"
                if data['PAYMENTS'].mean() - csv_data.iloc[0, 14] > 0 else "+")
            col1.metric(
                "总消费", csv_data.iloc[0, 14], "-"
                if data['BALANCE'].mean() - csv_data.iloc[0, 14] > 0 else "+")
            col1.metric(
                "可用预付现金", csv_data.iloc[0, 6],
                "-" if data['CASH_ADVANCE'].mean() - csv_data.iloc[0, 14] > 0
                else "+")
            col1.metric(
                "分期采购金额", csv_data.iloc[0, 5],
                "-" if data['INSTALLMENTS_PURCHASES'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")

            col2.metric(
                "余额更新率", csv_data.iloc[0, 2],
                "-" if data['BALANCE_FREQUENCY'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")
            col2.metric(
                "购买频率", csv_data.iloc[0, 7],
                "-" if data['PURCHASES_FREQUENCY'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")
            col2.metric(
                "预付现金频率", csv_data.iloc[0, 10],
                "-" if data['CASH_ADVANCE_FREQUENCY'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")
            col2.metric(
                "分期购买频率", csv_data.iloc[0, 9],
                "-" if data['PURCHASES_INSTALLMENTS_FREQUENCY'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")

            col3.metric(
                "信用卡额度", csv_data.iloc[0, 13],
                "-" if data['CREDIT_LIMIT'].mean() - csv_data.iloc[0, 14] > 0
                else "+")
            col3.metric(
                "预付现金交易次数", csv_data.iloc[0, 11],
                "-" if data['CASH_ADVANCE_TRX'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")
            col3.metric(
                "全额付款比率",
                str(csv_data.iloc[0, 16]) + "%",
                "-" if data['PRC_FULL_PAYMENT'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")
            col3.metric(
                "一次购买最高金额", csv_data.iloc[0, 4],
                "-" if data['ONEOFF_PURCHASES'].mean() -
                csv_data.iloc[0, 14] > 0 else "+")
            # mianinfo()
            st.subheader(" ")
    ...
    for tab in tabs:
        with tab:
            ...
            da = lis[i]
            chart = (altair.Chart(da).mark_bar().encode(
                x=altair.X(column_names[i],
